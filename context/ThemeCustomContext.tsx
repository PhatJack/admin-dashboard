...
const PRESET_KEY = 'shadcn/ui-admin-theme-preset';
const STYLE_TAG_ID = 'shadcn/ui-admin-theme-preset-styles';

export const ThemeCustomProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [currentPreset, setCurrentPreset] = useState<string>('ocean-breeze');
  ...
  useEffect(() => {
    ...
    if (savedPreset && defaultPresets[savedPreset]) {
      ...
      applyThemeToDOM(defaultPresets['ocean-breeze']);
    }
  }, []);
...
`;
  ...
  const resetTheme = useCallback(() => {
    setCustomTheme(null);
    setCurrentPreset('ocean-breeze');
    localStorage.removeItem(PRESET_KEY);

    // Apply default theme from preset
    const defaultPreset = defaultPresets['ocean-breeze'];
    if (defaultPreset) {
